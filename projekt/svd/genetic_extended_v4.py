...
class Genetic_extended_v4:
	...
	def mutate(Specimen):
		if np.random.rand() < 0.01:
			Specimen.S = Genetic_extended_v4.mutate_sigma(Specimen.S)
			Specimen.U = Genetic_extended_v4.mutate_orthogonal(Specimen.U)
			Specimen.Vt = Genetic_extended_v4.mutate_orthogonal(Specimen.Vt)
	...
	def calculate_svd(M, r):
		population = Genetic_extended_v4.create_population(M, r)
		for i in range(10000):
			fitness = []
			best_fit = 0
			best_S = 0
			for j in range(len(population)):
				fitness.append(Genetic_extended_v4.calculate_fitness(M, population[j]))
			...
			for j in range(0, len(population), 2):
				indexA, indexB = Genetic_extended_v4.select_parents(fitness, sum_of_fitness)
				ChildA, ChildB = Genetic_extended_v4.crossover(population[indexA], population[indexB])
				new_population.append(ChildA)
				new_population.append(ChildB)
			for j in range(len(population)):
				Genetic_extended_v4.mutate(new_population[j])
		...
		for i in range(len(population)):
			fitness.append(Genetic_extended_v4.calculate_fitness(M, population[i]))
