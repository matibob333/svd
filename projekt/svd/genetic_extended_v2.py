...
class Genetic_extended_v2:
	...
	def create_population(M):
		...
		for i in range(20):
			...
			U = (np.random.rand(rows, rows)*2)-1
			Vt = (np.random.rand(columns, columns)*2)-1
	...
	def calculate_fitness(M, specimen):
		...
		Res = np.subtract(M, A)
		norm1 = np.linalg.norm(Res)
		eye_rows = np.eye(np.size(M, 0))
		eye_columns = np.eye(np.size(M, 1))
		productU = specimen.U.dot(specimen.U.T)
		productVt = specimen.Vt.dot(specimen.Vt.T)
		Res = np.subtract(eye_rows, productU)
		norm2 = np.linalg.norm(Res)
		Res = np.subtract(eye_columns, productVt)
		norm3 = np.linalg.norm(Res)
		norm = (norm1+norm2+norm3)/3
	...
	def mutate(Specimen):
		if np.random.rand() < 0.01:
			Specimen.S = Genetic_extended_v2.mutate_sigma(Specimen.S)
			Specimen.U = Genetic_extended_v2.mutate_orthogonal(Specimen.U)
			Specimen.Vt = Genetic_extended_v2.mutate_orthogonal(Specimen.Vt)
	...
	@staticmethod
	def calculate_svd(M):
		population = Genetic_extended_v2.create_population(M)
		for i in range(1000):
			fitness = []
			best_fit = 0
			best_S = 0
			for j in range(len(population)):
				fitness.append(Genetic_extended_v2.calculate_fitness(M, population[j]))
			...
			for j in range(0, len(population), 2):
				indexA, indexB = Genetic_extended_v2.select_parents(fitness, sum_of_fitness)
				ChildA, ChildB = Genetic_extended_v2.crossover(population[indexA], population[indexB])
				new_population.append(ChildA)
				new_population.append(ChildB)
			for j in range(len(population)):
				Genetic_extended_v2.mutate(new_population[j])
		...
		for i in range(len(population)):
			fitness.append(Genetic_extended_v2.calculate_fitness(M, population[i]))
